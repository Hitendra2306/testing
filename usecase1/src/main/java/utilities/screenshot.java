...
public class screenshot {
	...
	public screenshot() throws IOException {
		...
		path = "D:/Hitendra/codesAutomation/usecase1/test-output/screenshots/luma-" + time;
		File f = new File(path);
		f.mkdir();
		// check if the directory can be created // using the specified path name
		if (f.mkdir() == true) {
			FileWriter writ = new FileWriter("D:/Hitendra/codesAutomation/usecase1/target/pathname.txt");
			writ.write(path);
			writ.close();
		}
